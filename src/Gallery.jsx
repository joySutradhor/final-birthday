...
export default function Gallery () {
  ...
  return (
    <section className='sectionSpace' id='gallery'>
      ...
      <p className='para pt-5 xl:w-1/3 pb-10'>{galleryDes}</p>
              ...
              <ImageListItem key={item.id} onClick={() => openMedia(item)}>
                <div className='image-container relative w-full h-auto overflow-x-hidden overflow-y-hidden cursor-pointer group '>
                  ...
                  {/* <div className='absolute z-50 inset-0 flex justify-center items-center group-hover:bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out'>
                    <h3 className='text-white text-xl font-bold font-custom'>
                      {item.title}
                    </h3>
                  </div> */}
                  ...
                  {item.zoom === 'false' && (
                    <div className='absolute inset-0  group-hover:opacity-100 group-hover:opacity-1 transition-opacity duration-300'>
                      ...
                      <div className='absolute inset-0 flex items-center justify-center bg-black bg-opacity-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300'>
                        <h3 className='text-white text-xs md:text-xl font-bold px-[5%]'>
                          {item.title}
                        </h3>
                      </div>
                    </div>
          ...
          >
            {currentMedia?.url || currentMedia?.videoUrl ? (
              <video
                ...
                loop
                muted
                className='w-[80vw] lg:w-auto h-full object-contain backdrop-blur-md'
              />
            ) : (
              <img
